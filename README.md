...
- **`cancel_subscription_now` approvals bind to `current_period_start_ts`, which is second-granular.** A withheld dual-signed cancellation stays valid if the subscription is closed and re-created within the same unix second as the observed period start. Both signatures plus close and re-subscribe must land in one second, and the replay cancels a subscription that has consumed no service.
